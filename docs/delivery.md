...
| --- | --- | --- | --- |
| Re-extract | `inbox.retry` (`id`, `revision`) | Reads the stored document again; concurrent clicks share one processing job | New revision; webhooks for the current endpoints; accounting as after any processing (a posted bill is never posted again or changed). Replaces corrections; the history keeps them |
