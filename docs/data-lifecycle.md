...
| --- | --- | --- | --- |
| Invoices, original documents, extraction, judgments, suppliers and their correction history, authorization sources with every version, their documents and import records, invoice-to-source match decisions, bank payment connections, transactions and payment decisions (optional; a disconnected bank's transactions only while a payment decision counts them), questions, question reruns and the answers they replaced, integration settings | while the workspace exists | a member deleting an invoice (file at once, record as below), the owner deleting the workspace ([offboarding](offboarding.md)) | none |
...
| `bank-feed.json` | bank payments (optional, [bank payments](bank-payments.md)): the workspace's bank connections (status, consent period, who gave it and when, expiry, last sync; never provider ids), accounts and transactions with their status |
| `payment-matches.json` | every payment decision about an exported invoice, oldest first per invoice: status, payment status, how and by whom it was made, the reason, the full result with its evidence, whether it is `current`, and its allocations (payment, bank charge or credit, with the transaction or credit note) |
...
| Salt Edge (optional bank payments; sandbox only today) | the workspace's Salt Edge customer, its bank connections and consents, and the bank data Salt Edge fetched | automatic: the customer is removed through the Salt Edge API, which removes its connections and revokes their consent. Salt Edge's own retention applies to what it holds |
