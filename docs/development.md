...
Accounting connections and draft-bill delivery use the self-hosted Nango
(`NANGO_BASE_URL`, `NANGO_PUBLIC_URL`, `NANGO_SECRET_KEY` and the Xero and
QuickBooks integration IDs, in both the API and standalone worker
environments). There is no default Nango host: left unset, both providers read
as unavailable. Verification points `NANGO_BASE_URL` at a loopback stub. See
[accounting integrations](accounting-integrations.md) for the provider
adapters, the local and sandbox proofs, and the provider apps still to be
registered.
