...
Retired or inherited and deliberately outside the product path: Midday's bank feeds
(GoCardless/Plaid/Teller) and bank-line transaction matching (the optional Salt Edge
bank payments in `docs/bank-payments.md` are separate and never use them), time tracking,
...
- **Bank payments (optional, #9):** off per workspace by default; Salt Edge AIS (read-only consent, bank sign-in on Salt Edge) connects a bank to the workspace's own Salt Edge customer, a chained `sync-bank-connection` job pulls from durable per-account cursors (pending/posted/superseded/reversed), and `match-payments` records immutable `invoice_payment_matches` (+ `_allocations`; `inbox.payment_match_id`), kept apart from source matching. Only a printed invoice reference asserts paid; never across currencies. Production stays off (`BANK_PAYMENTS_ENABLED`, and no live Salt Edge app); see `docs/bank-payments.md`
...
`inbox_redeliveries`, `invoice_corrections`, `delivery_policies`, `delivery_decisions`, `authorization_sources` (+ `_versions`, `_documents`, `_imports`), `invoice_source_matches` (+ `invoice_source_links`, `invoice_source_allocations`), `audit_events`, `bank_payment_settings`, `bank_feed_connections` (+ `_accounts`, `_transactions`), `invoice_payment_matches` (+ `invoice_payment_allocations`) and `workflow_jobs`. Bank and transaction tables are
