...
- **Reconciliation (Layer 2):** a `reconcile-invoice` job per match decision × revision compares allocations with the cited version (exact BigInt decimals, published tolerances, never across currencies; TypeSafe only judges unpaired lines' scope, never numbers) and records immutable `invoice_reconciliations` + `invoice_source_consumption` under per-source row locks; a source's balance is derived from current reconciliations only (never stored). Opt-in `authorization_*` delivery rules make a revision's decision `pending` until reconciled. See `docs/reconciliation.md`
...
`inbox_redeliveries`, `invoice_corrections`, `delivery_policies`, `delivery_decisions`, `authorization_sources` (+ `_versions`, `_documents`, `_imports`), `invoice_source_matches` (+ `invoice_source_links`, `invoice_source_allocations`), `audit_events` and `workflow_jobs`. Bank and transaction tables are
