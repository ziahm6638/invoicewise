...
- **Integrations:** self-hosted Nango on hp-slice for Xero/QuickBooks (auth + proxy only, so bill adapters live in `packages/jobs`; see `docs/accounting-integrations.md`), Polar (billing), API/MCP/webhooks. Nango integrations are created from Infisical with `bun run nango:configure-integration`, never by hand. QuickBooks has no draft bills (open, unpaid `Bill`/`VendorCredit`), so a connection posts only after an admin's setup and confirmed opt-in (`auto_post_enabled_at`); its create is idempotent by `requestid` plus a DocNumber/vendor lookup
