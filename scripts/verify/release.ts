...
async function workflowAndStorageVerifiers() {
  ...
  await v.runStep("verify:jobs-quickbooks", {
    command: "bun",
    args: ["--no-env-file", "run", "verify:quickbooks"],
    cwd: ws(JOBS_DIR),
    env: env({
      DATABASE_PRIMARY_URL: databaseUrl(JOBS_DATABASE),
      LOCAL_STORAGE_PATH: join(v.tmpDir, "jobs-storage"),
  ...
  await v.runStep("verify:jobs-suppliers", {
    command: "bun",
    args: ["--no-env-file", "run", "verify:suppliers"],
    cwd: ws(JOBS_DIR),
    env: env({
      DATABASE_PRIMARY_URL: databaseUrl(JOBS_DATABASE),
      LOCAL_STORAGE_PATH: join(v.tmpDir, "jobs-storage"),
  ...
  await v.runStep("verify:jobs-exceptions", {
    command: "bun",
    args: ["--no-env-file", "run", "verify:exceptions"],
    cwd: ws(JOBS_DIR),
    env: env({
      DATABASE_PRIMARY_URL: databaseUrl(JOBS_DATABASE),
      LOCAL_STORAGE_PATH: join(v.tmpDir, "jobs-storage"),
  ...
  await v.runStep("verify:jobs-questions", {
    command: "bun",
    args: ["--no-env-file", "run", "verify:questions"],
    cwd: ws(JOBS_DIR),
    env: env({
      DATABASE_PRIMARY_URL: databaseUrl(JOBS_DATABASE),
      LOCAL_STORAGE_PATH: join(v.tmpDir, "jobs-storage"),
  ...
  await v.runStep("verify:jobs-source-matching", {
    command: "bun",
    args: ["--no-env-file", "run", "verify:source-matching"],
    cwd: ws(JOBS_DIR),
    env: env({
      DATABASE_PRIMARY_URL: databaseUrl(JOBS_DATABASE),
    }),
  ...
  await v.runStep("verify:jobs-reconciliation", {
    command: "bun",
    args: ["--no-env-file", "run", "verify:reconciliation"],
    cwd: ws(JOBS_DIR),
    env: env({
      DATABASE_PRIMARY_URL: databaseUrl(JOBS_DATABASE),
      LOCAL_STORAGE_PATH: join(v.tmpDir, "jobs-storage"),
...
async function securityRegressionSuites() {
  ...
  await v.runStep("verify:data-lifecycle", {
    command: "bun",
    args: [
      "--no-env-file",
      "test",
      "src/trpc/routers/data.lifecycle.integration.test.ts",
    ],
    cwd: ws(API_DIR),
    env: env({
      PERMISSIONS_TEST_DATABASE_URL: databaseUrl(PERMISSIONS_DATABASE),
