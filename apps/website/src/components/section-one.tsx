export function SectionOne() {
  return (
    <section className="mt-24 md:mt-[200px] mb-12" id="how-it-works">
      <h3 className="text-4xl md:text-8xl font-medium">
        Invoices in. Structured data out.
      </h3>
      <p className="mt-4 md:mt-8 text-[#878787] max-w-[720px]">
        InvoiceWise is middleware: it receives your invoices by email, extracts
        the fields with TypeSafe, answers your questions and delivers clean,
        structured data to the tools you already use.
      </p>
    </section>
  );
}
