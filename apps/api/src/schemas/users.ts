...
/**
 * Generic profile fields only.
 *
 * `email` is intentionally not accepted here and the object is strict, so a
 * client that sends it gets a validation error instead of silently believing
 * an address changed. Verified addresses change only through the authoritative
 * Better Auth email-change flow (#33).
 */
