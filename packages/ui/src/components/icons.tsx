...
import { BRAND_LOGO, BRAND_MARK } from "../brand";
...
/** The InvoiceWise symbol from `../brand`, drawn in the current text colour. */
function BrandMark() {
  const { path, dot, strokeWidth } = BRAND_MARK;
  return (
    <>
      <path
        d={path}
...
export const Icons = {
  LogoSmall: ({ size = 28, className }: SVGIconProps) => (
    <svg
      width={size}
      height={size}
      viewBox={`0 0 ${BRAND_MARK.size} ${BRAND_MARK.size}`}
      role="img"
  ...
  Logo: (props: React.SVGProps<SVGSVGElement>) => (
    <svg
      ...
      viewBox={`0 0 ${BRAND_LOGO.width} ${BRAND_LOGO.height}`}
      role="img"
      aria-label="InvoiceWise"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <BrandMark />
      <path fill="currentColor" d={BRAND_LOGO.wordmarkPath} />
    </svg>
  ),
